...
if(! function_exists('pullOutImage')) {
    function pullOutImage($content, $boardId=0, $writeId=0, $file=0)
    {
        ...
        // 첨부파일에서 이미지 가져오기
        if(!$flag && $file) {
            $boardTagged = app()->tagged('board');
            $boardModel = null;
            foreach ($boardTagged as $board) {
                $boardModel = $board;
                break;
            }
...
if (! function_exists('deleteWritePoint')) {
    ...
    {
        $writeTagged = app()->tagged('write');
        $boardTagged = app()->tagged('board');
        
        $writeModel = null;
        foreach ($writeTagged as $write) {
            $writeModel = $write;
            break;
        }
...
if (! function_exists('urlAutoLink')) {
    function urlAutoLink($str)
    {
        ...
        $str = preg_replace("/([^(href=\"?'?)|(src=\"?'?)]|\(|^)((http|https|ftp|telnet|news|mms):\/\/[a-zA-Z0-9\.-]+\.[가-힣a-zA-Z0-9\.:&#!=_\?\/~\+%@\-\|\,\(\)]+)/i", "\\1<a href=\"\\2\" target=\"$target\" rel=\"nofollow\">\\2</a>", $str);
