...
class BoardsEventListener
{
    ...
    {
        $writeTaggedTagged = app()->tagged('write');
$writeTagged = null;
foreach ($writeTaggedTagged as $item) {
    $writeTagged = $item;
    break;
}
        $boardTaggedTagged = app()->tagged('board');
$boardTagged = null;
foreach ($boardTaggedTagged as $item) {
    $boardTagged = $item;
    break;
}
